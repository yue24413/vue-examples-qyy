...
import { listDapartmentService, listTeacherService } from './service'
listDapartmentService
...
// 所有部门，因无需改变定义为非响应式
const departments = await listDapartmentService()
...
watch(departmentR, async () => {
  teacherR.value = {}
  teachersR.value = []
  departmentR.value?.id && (teachersR.value = await listTeacherService(departmentR.value.id))
})
...
/**
 async/await 语法
    async/await 是一种处理 JavaScript 异步操作的方法，它使得异步代码看起来更像同步代码，易于理解和编写。async/await 需要与 Promise 一起使用。

    async: 用于声明一个函数是异步的。这样的函数会返回一个 Promise 对象。
    await: 只能在 async 函数内部使用，用来等待一个 Promise 对象完成（解析或拒绝）。

    await关键字会让当前的 async 函数暂停执行，直到 listDepartmentService() 返回的 Promise 解析完成。
     当 Promise 解析后，其解析的结果值将被赋给 Departments 变量。
 */
</script>
<template>
  <div>
